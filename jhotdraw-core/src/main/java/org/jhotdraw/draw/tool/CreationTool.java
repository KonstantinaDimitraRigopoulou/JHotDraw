...
public class CreationTool extends AbstractTool {
    ...
    public void mouseReleased(MouseEvent evt) {
        ...
        if (isZeroSize(createdFigure)) {
            removeCreatedFigure();
            finishToolIfNeeded();
            return;
        }

        enforceMinimalSizeIfClick(evt, createdFigure);
        layoutIfComposite(createdFigure);
        fireUndoableCreation(createdFigure, getDrawing());
        maybeInvalidateBounds(evt);
    ...
    private boolean isZeroSize(Figure figure) {
    ...
    private void enforceMinimalSizeIfClick(MouseEvent evt, Figure figure) {
        ...
        figure.willChange();
        figure.setBounds(
                constrainPoint(new Point(anchor.x, anchor.y), figure),
    ...
    private void layoutIfComposite(Figure figure) {
        if (figure instanceof CompositeFigure) {
            ((CompositeFigure) figure).layout();
        }
    }

    private void fireUndoableCreation(final Figure figure, final Drawing drawing) {
    ...
    private void maybeInvalidateBounds(MouseEvent evt) {
